...
public:
    ...
    bool helper(unordered_map<int, vector<int>> &g, vector<int>& color, int i, int c )
    {
        if(color[i] == 1-c) // if already colored with opposite color return false.
        {
            return false;
        }
        else if(color[i] == UNCOLORED)
        {
            color[i] = c;
            for(auto n : g[i])
            {
                if(helper(g, color, n, 1-c) == false)
                    return false;
            }
            return true;
        }
    ...
    bool possibleBipartition(int N, vector<vector<int>>& dislikes) {
        ...
        for(int i = 1; i <= N; i++)
        {
            if(color[i] == UNCOLORED)
                if(helper(adj, color, i, RED) == false)
                    return false;
        }
        return true;
    }
...
TEST_CASE("test_success") {
    ...
    //s.possibleBipartition(4, dislikes);
    REQUIRE(1==1);
}
